...
class Teams:
    ...
    async def invite(self, ctx, user: discord.User):
        ...
        if username == None:
            ...
            return
        #get the author's owned teams
        utils.database.execute(f"SELECT team_id, team_name, game FROM teams WHERE owner_id='{ctx.author.id}' AND game IN unnest(SELECT games FROM servers WHERE server_id='{ctx.guild.id}');")
        owned_teams = utils.database.fetchall()
