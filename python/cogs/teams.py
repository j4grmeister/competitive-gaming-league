...
class Teams:
    ...
    async def createteam(self, ctx, *, team_name):
        ...
        if len(guild_games) == 1:
            ...
            utils.database.execute(f"INSERT INTO team_table (team_id, game, team_name, team_elo, primary_players) VALUES ({teamid}, '{guild_games[0]}', '{eteam_name}', '{team_elo}', '{{\"{ctx.author.id}\"}}');")
